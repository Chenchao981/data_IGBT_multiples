...
Features:
- Riyuexin: DC, FT static scatter/box charts, DVDS, RG, PAT, and SYL&SBL
- TMS adapters: independent, fail-closed Riyuexin and Riyueguang FT DC routes
- Jiequn: DC-AI, DC-1, DC-unified, DC-3, FT static scatter/box charts, DVDS, RG, PAT, and SYL&SBL
- Dianji: FT-ALL cleaning, FT static scatter/box charts, PAT, and SYL&SBL
...
- PAT for Riyuexin, Riyueguang, Jiequn, Dianji, and Jijia previews a raw-source directory and calculates directly with bounded memory; no cleaned workbook is required.
- Riyuexin and Riyueguang PAT accept a product root with DC/DVDS/RG subdirectories or one raw type directory. Riyueguang keeps its independent reviewed EBR/DC layouts. Dianji PAT accepts one registered raw format directory and rejects mixed formats or products. Jijia PAT accepts the reviewed STS8203 CSV product directory.
- SYL&SBL accepts one .xls or .xlsx yield workbook.
- Output files are saved under the selected output directory.
- After a supported Riyuexin, Jiequn, or Dianji cleaning operation succeeds, click Scatter / Box Charts to open the FT chart page.
...
- Parameters start unselected. Use Select All or choose multiple individual parameters, then draw the charts.
- Each parameter/chart type has a compact inline Y-axis toolbar above its image. Enter minimum and maximum, then Apply to redraw immediately; no checkbox is required. Use Restore Automatic Range to reset the viewport. The download uses the same PNG.
