...
4. 首次打开只加载参数筛选。选择参数和图表类型后点击“绘制图形”，页面才逐参数生成静态 PNG；筛选改变后需要重新点击绘图，不沿用旧图。
...
8. 在参数图上方的单行操作栏勾选“自定义 Y 轴范围”，输入最小值和最大值，点击右侧“应用自定义并绘制”。支持负数、小数和科学计数法，单位与该参数一致；每个参数和两类图独立保存设置。取消勾选并应用后恢复自动范围。自定义范围只调整显示与下载图片，全部有效值仍参与统计。
