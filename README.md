...
| 封装厂 | 数据格式 | 数据处理 | 状态 |
|--------|----------|----------|------|
| **日月新 (Riyuexin)** | .xlsx 分目录 | DC / FT 静态散点图与箱体图 / DVDS / RG | ✅ 稳定 |
| **杰群 批次1** | .csv 分目录 | DC / FT 静态散点图与箱体图 / DVDS / RG | ✅ 稳定 |
| **杰群 批次2** | .csv 统一CSV | DC+DVDS+RG 合并 / FT 静态散点图与箱体图 | ✅ 稳定 |
| **杰群 第三产线** | .csv 产品目录平铺、可变尾空列 | DC / FT 静态散点图与箱体图 | ✅ 可用 |
| **杰群清洗** | DC-AI + 专用 DVDS/RG | 自动完整清洗或单独清洗 DVDS/RG / FT 静态散点图与箱体图 | ✅ 可用 |
| PAT 参数分析 | 日月新 / 杰群 / 电基原始文件目标目录 | 低内存逐文件统计汇总 | ✅ 可用 |
| **电基 (Dianji)** | PowerTECH 伪 `.xls` / 原生 `.xlsx` / STS8203 `.csv` / DP1205 TF `.csv` | FT-ALL 自动识别清洗 / FT 静态散点图与箱体图 | ✅ 可用 |
...
│
├── frontend/
│   ├── ft_scatter.py               ← FT 图表数据包、清单与旧接口兼容逻辑
│   ├── ft_static_charts.py         ← FT 专用静态 PNG 散点图与箱体图渲染
│   └── ft_scatter_app.py           ← Streamlit 参数筛选、图片显示与 PNG 下载
│
├── gui/
...
### FT 参数静态散点图与箱体图（日月新 / 杰群 / 电基，v2.22）

1. 选择支持 FT 图表的清洗入口：日月新 `DC`、杰群 `DC-AI`，或电基 `FT-ALL`。
2. 选择原始数据目录和输出目录后执行清洗。
3. 清洗成功后，“散点图 / 箱体图”按钮自动启用；点击后打开浏览器页面。
4. 首次打开参数默认不选，可点击“全选”或自行多选。选择参数和图表类型后点击“绘制图形”，页面才逐参数生成静态 PNG；筛选改变后需要重新点击绘图，不沿用旧图。
5. FT 始终以完整 `lot_ID` 作为批次身份和箱体分组；`NUM` 只用于批次内样本的稳定展示顺序，`Source_ID` 只用于来源追溯和规格绑定。不会生成、读取或套用 CP 的 `Wafer_ID` 分组。
6. 散点图全量绘制有限测量值，不抽样；箱体图保持每个完整 `lot_ID` 一个箱体，不按 `Source_ID` 拆箱。默认纵轴聚焦主体分布，也可分别切换散点图或箱体图的完整纵轴范围，切换只改变视窗，不改变原始值和统计口径。
7. 规格来自各 `Source_ID` 对应源文件的上下限、单位和测试条件；清洗时的单位换算同步应用到规格。不同批次使用稳定且可扩展的颜色，图片采用白底亮色主题，并可逐参数下载 PNG。
8. 在参数图上方的单行操作栏输入最小值和最大值，直接点击右侧“应用自定义并绘制”，无需勾选复选框。支持负数、小数和科学计数法，单位与该参数一致；每个参数和两类图独立保存设置。点击“恢复自动范围”恢复自动绘图。自定义范围只调整显示与下载图片，全部有效值仍参与统计。

清洗阶段利用已经读入内存的数据生成便携散点数据包。日月新和杰群的批次目录使用
`ft_scatter_data.csv.gz`、`ft_scatter_spec.csv` 和 `ft_scatter_manifest.json`；电基先创建
`<产品主体>_NNN` 流水目录（如 `NCEAP016N85LL(M)_001`），再把清洗 Excel 与
`<清洗文件名>_ft_scatter_*` 一起放入该目录。点击“散点图 / 箱体图”时读取本次清洗返回的明确清单，
不会重新扫描原始文件，也不会猜测“最新文件”。图片在内存中逐参数生成，不创建临时图形文件；
空值、非数值和无穷值不会补成零或参与绘制。

### 杰群面板说明

第一行保留一个 DC 自动入口和两个专用数据入口：

| 按钮 | 处理的数据格式 | 输入目录示例 | 输出 |
|------|---------------|-------------|------|
| **DC-AI** | 自动判断 DC-1 / DC-统一CSV / DC-3 | 指向一个产品根目录、RAW 目录、DC 目录或第三产线产品目录 | 自动清洗实际存在的 DC/DVDS/RG |
| **DVDS** | 专用 DVDS 分目录数据 | 直接选择 `DVDS` 目录，或选择只含一个 DVDS 子目录的产品目录 | NUM + 批次 + DVDS(mV) |
| **RG** | 专用 RG 分目录数据 | 直接选择 `RG` 目录，或选择只含一个 RG 子目录的产品目录 | NUM + 批次 + RG(R) |
...
`DC-AI` 不能把一个纯 DVDS 或纯 RG 目录识别为 DC 格式。需要单独处理专用目录时，
必须使用 `DVDS` 或 `RG` 按钮；这两个入口继续调用原来经过验证的专用清洗器。
如果误选的是已经生成的单批次输出目录，并且其中只有一个结构正确的 `DVDS_Data`
或 `RG_Data` 工作簿，程序会识别现成结果并跳过重复清洗，不再继续寻找子目录。
...
| PyQt5 | GUI 界面 |
| Plotly | 旧 FT 散点图接口兼容 |
| Matplotlib | FT 静态散点图与箱体图 PNG 渲染 |
| Streamlit | 本地 FT 图表筛选、显示与下载页面 |

## 🔄 版本历史

- **v2.22.2** (2026-09-08)：自定义 Y 轴无需复选框，输入后直接应用；增加“恢复自动范围”。参数默认空选，支持“全选”和自由多选；常用发布入口同步更新。
- **v2.22.1** (2026-09-08)：Y 轴设置改为图上方单行操作栏，移除折叠框和图前解释文字，右侧“应用自定义并绘制”直接更新图形；保留参数、图型独立设置和原统计口径。新版独立交付于 `packaging/release/FT_Data_Cleaner_v2.22.1/`。
- **v2.22.0** (2026-09-08)：散点图和箱体图支持按参数独立输入 Y 轴最小值、最大值，应用后按精确范围绘制及下载 PNG；支持负数、小数、科学计数法，输入无效时明确提示，取消自定义可恢复自动范围。新版独立交付于 `packaging/release/FT_Data_Cleaner_v2.22.0/`。
- **v2.21.0** (2026-09-07)：FT 参数图表升级为逐参数静态 PNG 散点图与箱体图，支持首次筛选后按需绘制、默认主体聚焦、两类图独立切换完整纵轴及 PNG 下载。完整 `lot_ID` 保持为批次与箱体分组，`NUM` 只控制批次内展示顺序，`Source_ID` 只绑定来源规格和追溯；不生成或套用 CP 的 `Wafer_ID` 分组，清洗、Bin、良率、PAT 与 Cpk 口径不变。
- **v2.19.0** (2026-09-01)：电基 PowerTECH 伪 `.xls` 与原生 `.xlsx` 改为按 `Item Name + Bias1-3 + Unit` 动态识别业务参数，不再以固定 32/34/35/38/39 项列数作为解析门槛；右侧新增参数自动进入 RAW、规格、散点和 PAT，旧文件缺少的新参数补空值。`CONT*`、`SAME`、`DELAY` 等设备控制/占位项继续排除，身份、程序、单位和参数语义冲突仍失败关闭；同时登记 dj9 的 M08M15 精确程序。
- **v2.18.0** (2026-08-28)：兼容电基 `dj8` 已验证的 PowerTECH 文本命名：外部 `.xls` 文件名可省略产品并带数字复制后缀 `(n)`，但仅在文件名、`DataFileName`、精确登记的 `TestFileName` 程序和 `Lot` 身份完全一致时放行。真实样本 6,528 条源记录按有效 `DVDS(mV)` 保留 6,383 条，原有 34 项/19 参数契约不变。
- **v2.17.0** (2026-08-27)：为 TMS 正式入库增加 Lot 缺失人工补录接口。仅当日月新/日月光文件的厂家、Product、Source、时间与 Workbook 布局均符合已批准签名、唯一缺少 Lot 时才请求人工值；补录值只填空、不覆盖冲突的源 Lot，并同步进入清洗明细、散点数据、Spec 与 Manifest，原始 XLSX 不修改。未知文件名继续失败关闭。
...
- **v2.7.3** (2026-07-22)：电基 FT-ALL 新增第二种已验证 PowerTECH 34 项布局；兼容 Item 29–31 为 `IGSS(-VGS)/IGSS(+VGS)/IDSS`，并按参数名与偏置条件恢复统一 `ISGS/IGSS/IDSS` 输出顺序，继续拒绝未经验证的排列。
- **v2.7.2** (2026-07-22)：日月新 FT 散点图改为亮色主题；不同 `lot_ID` 批次使用独立颜色并在右侧图例中标识；放大散点、参数标题、坐标刻度及 LSL/USL 标签；按批次分层抽样，避免小批次及其图例被抽样丢失。
- **v2.7.1** (2026-07-22)：优化日月新 FT 散点图；GUI 按钮改为左侧“开始清洗”、右侧“FT散点图”，参数名称移到坐标轴上方，颜色和顶部图例按 `lot_ID` 批次区分；压缩规格内重复点并保留全部超限点，真实 DC-6 的 18 图构建由约 5.50 秒降至 1.21 秒。
...
**`panels/jiequn_panel.py`：** 第一行保留 DC-AI、DVDS、RG 清洗入口，第二行是 PAT 参数分析，第三行是 SYL&SBL 良率分析。
- 输入/输出默认路径与日月新一致，启动后都指向用户桌面。
- `DC-AI` 是 DC 主入口，调用 `dc_auto.run_auto_dc(inp, out)`；具体 DC 格式和产品根目录中的 DVDS/RG 配对由后端确定性识别。
- `DVDS`、`RG` 保留为纯专用目录入口，分别调用既有专用清洗器。
...
| `gui/panels/riyuexin_panel.py` | `RiyuexinPanel` | DC/DVDS/RG + PAT/SYL&SBL |
| `gui/panels/jiequn_panel.py` | `JiequnPanel` | DC-AI + 5个手工清洗入口 + PAT/SYL&SBL |
| `gui/panels/dianji_panel.py` | `DianjiPanel` | FT-ALL 清洗 + PAT/SYL&SBL |
