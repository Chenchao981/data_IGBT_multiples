...
5. 规格来自每个源文件自己的上下限和 Bias 条件；杰群的 A→nA、R→mR 与电基的源单位换算会同步应用到规格线。P 型程序若以反向数值记录 Min/Max，则保留原始文本，同时按真实数值下界/上界用于 LSL/USL 和超限判断。
6. 不同颜色按批次区分，图例显示在图形右侧；页面采用亮色背景，并放大散点、参数标题、坐标数字及 LSL/USL 标签。
...
`<清洗文件名>_ft_scatter_*` 一起放入该目录。点击散点图时读取本次清洗返回的明确清单，
不会重新扫描原始文件，也不会猜测“最新文件”。单图默认最多显示 4,000 个代表点；
同一批次、同一数值的规格内重复点会均匀压缩，但全部超限点始终保留。
