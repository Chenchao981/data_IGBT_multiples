...
从 CSV 文件中提取 DVDS_EX 参数，合并后输出为统一格式的 Excel 文件。
DVDS 无单位换算需求（单位已是 V）。
"""
...
class JiequnDVDSCleaner(BaseCleaner):
    ...
    def _get_subdir(self) -> Path:
        ...
        for p in base.rglob(sub):
            if p.is_dir():
                if list(p.glob("*DVDS*.csv")) or list(p.glob("*DVDS*.CSV")):
                    return p
        direct = base / sub
        if direct.exists():
            return direct
        raise FileNotFoundError(f"未找到 DVDS 数据目录: {base} / {sub}")

    def process_all(self, data_type: str = None) -> bool:
        logger.info("=" * 50)
        logger.info("杰群 DVDS 数据清洗 (CSV)")
        logger.info("=" * 50)

        try:
            dvds_dir = self._get_subdir()
            logger.info(f"DVDS 数据目录: {dvds_dir}")

            csv_files = sorted(set(dvds_dir.glob("*DVDS*.[cC][sS][vV]")))
            if not csv_files:
                csv_files = sorted(set(dvds_dir.glob("*.[cC][sS][vV]")))
                csv_files = [f for f in csv_files if 'DVDS' in f.name.upper()]

            logger.info(f"找到 {len(csv_files)} 个 DVDS CSV 文件")

            if not csv_files:
                logger.error("未找到 DVDS CSV 文件")
                return False

            all_dfs = []
            for f in csv_files:
                df = parse_dta_csv(str(f), DVDS_PARAMS)
                if df is not None and not df.empty:
                    all_dfs.append(df)

            if not all_dfs:
                logger.error("未能提取到 DVDS 数据")
                return False
            ...
            filename = generate_lot_based_filename(lot_ids, "DVDS_JQ")
            output_path = self.output_dir / filename
            success = write_excel_fast(merged, output_path, sheet_name='DVDS_Data_JQ')

            if success:
                logger.info(f"杰群 DVDS 数据保存成功: {output_path}")
            ...
            logger.error(f"杰群 DVDS 清洗出错: {e}", exc_info=True)
            return False


if __name__ == "__main__":
    cleaner = JiequnDVDSCleaner()
    cleaner.process_all()
